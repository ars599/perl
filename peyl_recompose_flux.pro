...
FUNCTION PEYL_RECOMPOSE_FLUX, file_in=file_in, $
                              ...
                              special_no_split_base=special_no_split_base

if keyword_set(old_region) then begin
    print,'use old regions...'
endif else begin
    old_region = 0
endelse
...
if not ccg_vdef(special_no_split_base) then special_no_split_base = 1
...
if split_flux ge 1 then begin
    ...
    ii = sort(temp)
    uu = uniq(temp(ii))
    if (n_elements(uu) eq 1 and temp(0) ne 'pixel') then begin
        print,'bizarre seulement 1 processus avec option split_flux!!'
        if peyl_pause() then stop
    endif
    trac_tab = temp(ii(uu))
    foo = where(trac_tab ne 'pixel',cc)
    if cc eq 0 then trac_tab = ['land','oce'] else begin
        foo2 = where(trac_tab eq 'land' or trac_tab eq 'oce', cc2)
        if cc2 gt 0 then message,'Problem: process land/oce deja utilise!!!'
        trac_tab =['land','oce',trac_tab(foo)]
    endelse
...
if split_flux ge 1 or add_sealand_mask then begin
    if old_region then begin
        indoce = peyl_choixregnew_old('oce', grid_name=grid_name,$
                                      noinfo=1, extend_reg=0,$
                                      grid_pixel=grid)
        indland = peyl_choixregnew_old('land', grid_name=grid_name,$
                                       noinfo=1, extend_reg=0,$
                                       grid_pixel=grid)
    endif else begin
        indoce = peyl_choixregnew('std_oce', grid_name=grid_name,$
                                  noinfo=1, extend_reg=0,$
                                  grid_pixel=grid)
        indland = peyl_choixregnew('std_land', grid_name=grid_name,$
                                   noinfo=1, extend_reg=0,$
                                   grid_pixel=grid)
    endelse
    ...
    if add_sealand_mask then mask(indland.ii2d) = 1                 
endif

;--------------- Boucle sur toutes les sources
flag_use = 0
if n_elements(source_to_use) gt 0 then $
  source_to_use_flag = intarr(n_elements(source_to_use))
for ns=0,nsrce-1 do begin
    ...
    if n_elements(source_to_use) gt 0 then begin
        if (source_to_use(0) ne 'all' and source_to_use(0) ne '') then begin
            ...
            source_to_use_flag(ii(0)) = 1
        endif else source_to_use_flag(0) = 1
    endif 
...
;-------------- Source regionale..
        ...
        regall = peyl_concatstr(source_base(iicomp_base).reg,separator='+')
                                ...
                                ;---- SPECIAL POUR NE PAS TENIR COMPTE
                                ;     DE LA SOUS-DISTRIBUTION LIEE AUX
                                ;     SOURCES DE BASE ET APPLIQUER UN
                                ;     SCALING GLOBAL POUR LA REGION
                                ;     CONSIDEREE! 
        if special_no_split_base then begin
            nbcomp = 1
        endif 
        ...
        for nb=0,nbcomp-1 do begin
            ...
            if special_no_split_base eq 0 then begin            

                iireg = iicomp_base(nb)
                reg_curent = source_base(iireg).reg
                ...
                reg_curent = regall
                iireg = 0 ;--- only used for a print below..
            endelse
            ...
            tempo_time_bis = tempo_time_bis(foo)
                                ...
                                ;    consideree..
            if old_region then begin
                ind = peyl_choixregnew_old(reg_curent, grid_name=grid_name,$
                                           grid_pixel = grid, /noinfo)
            endif else begin
                ind = peyl_choixregnew(reg_curent, grid_name=grid_name,$
                                       grid_pixel = grid, /noinfo)
            endelse
            ...
            if nb eq 0 then begin
                if old_region then begin
                    indall = peyl_choixregnew_old(regall, grid_name=grid_name,$
                                                  grid_pixel=grid,/noinfo)
                endif else begin
                    indall = peyl_choixregnew(regall, grid_name=grid_name,$
                                              grid_pixel=grid,/noinfo)
                endelse
                ...
                if im lt 24 then begin
                    print,im,im1,im_file,flux_reg,flux_allreg,$
                      ...
                      source(ns).flux_poste(im1)
                                ...
                                ;---- CAS SIOUX AVEC RESCALING
                                ;     PROPORTIONEL AU FLUX DE BASE...
                if special_no_split_base eq 0 then begin            
                    ...
                    endelse
                                ...
                                ;--- SPECIAL NO SPLIT DES FLUX DE
                                ;    BASE... 
                endif else begin
                    print,'Rescaling simple....'
                    coef_poste = source(ns).flux_poste(im1)
                    coef_prior = source(ns).flux(im1) 
                endelse
...
;------ Check sur source_to_use
if n_elements(source_to_use) gt 0 then begin
    foo = where(source_to_use_flag eq 0, cc)
    if cc gt 0 then print,'WARNING SOURCE_to_USE not used : ',source_to_use(foo)
endif
