...
;;        nbcomp      = n_elements(components)
        ind         = peyl_choixregnew(reg_map(i),grid_name=grid.name,$
...
sz=size(data_map) ; number of dims then size of each dim

px = !x.window * !d.x_vsize ; !d structure of actual graphic device
                            ; window : index of currently open window
py = !y.window * !d.y_vsize ; !x structures of type AXIS, that affect the appearance and scaling of the three axes
swx = px(1)-px(0)               ; Size in x in device units

swy = py(1)-py(0)               ; Size in Y
six = float(sz(1))              ; Image dimension sizes
siy = float(sz(2))
...
;;    tv,  data_map,  px(0),py(0),  xsize = swx, ysize = swy,  /device
    warp=map_image(data_map,xx,yy,xs,ys,compress=1,latmin=limit(0),latmax=limit(2),$
                   lonmin=limit(1),lonmax=limit(3),missing=!P.BACKGROUND,scale=0.04)
...
;;        nbcomp      = n_elements(components)
        ind         = peyl_choixregnew(reg_contour(i),grid_name=grid.name,$
                                       special_aerocarb=special_aerocarb,/noinfo) 
