...
if new_sim eq 0 then begin
    ...
    if not keyword_set(path) then path = ['/home/carbone/peylin/COCO2/','/home/assimil/peylin/COCO2/']
    trac_tab = [$
                 ['inv_ctrl','inv_ctrl']$
...
if 1 then begin
    ...
    ;-------------------------------------------------
    ;  DEFINITION DE LA GRILLE  TM3
    ;-------------------------------------------------

    grid_tm3=peyl_choixgrid('TM3',noinfo=noinfo)
    nlon_tm3 = grid_tm3.nlon
    lon_tm3  = grid_tm3.lon
    dlon_tm3 = grid_tm3.dlon
    lat_tm3  = grid_tm3.lat
    dlat_tm3 = grid_tm3.dlat
    nlat_tm3 = grid_tm3.nlat
...
;help,p_mid,p_surf
    ...
    ;---------------------------------------------------------
    ;  LECTURE DES PRESSIONS DE SURFACES ET COEFF BP ET AP POUR LMDZ
    ;---------------------------------------------------------
    ...
    file = '/home/atmosphere/idelkadi/simulations_lmdzt/start.nc'
                                ...
                                ;   moyenne temporelle
    peyl_readnc,p_surf_lmdz,file=file,var_name='ps'
    p_surf_lmdz = p_surf_lmdz(0:nlon-1,*,*)
    if (size(p_surf_lmdz))(0) eq 3 then $
      p_surf_lmdz = total(p_surf_lmdz,3)/n_elements(p_surf_lmdz(0,0,*))
    if file_glo eq 0 then begin
        p_surf_lmdz = p_surf_lmdz(*,iilat_restrict)
    endif
    ...
    p_interm_lmdz = fltarr(nlon,nlat,nlev+1)
    delta_p_lmdz  = fltarr(nlon,nlat,nlev)
    for n=0,nlev do begin
        p_interm_lmdz(*,*,n) = bp(n) * p_surf_lmdz + ap(n) 
        if n gt 0 then delta_p_lmdz(*,*,n-1) = p_interm_lmdz(*,*,n-1)-p_interm_lmdz(*,*,n)
    endfor
    
                                ;-- Calcul pression mid 
    p_mid_lmdz = fltarr(nlon,nlat,nlev)
    for n=0,nlev-1 do p_mid_lmdz(*,*,n) = (p_interm_lmdz(*,*,n)+p_interm_lmdz(*,*,n+1)) / 2.

    ;---------------------------------------------------------
    ;  LECTURE DES PRESSIONS DE SURFACES ET COEFF BP ET AP POUR TM3
    ;---------------------------------------------------------

    file = '/home/geoatm/peylin/SATELLITE/COMP_TOVS/COMP_DIRECT_TOVS/ps_TM3.nc'

    ;-- lecture P_surf (retrait de la
    ;   longitude additionnelle) et
    ;   moyenne temporelle
    peyl_readnc,p_surf_tm3,file=file,var_name='m001'
    p_surf_tm3 = p_surf_tm3(0:nlon_tm3-1,*,*,*)

    if (size(p_surf_tm3))(0) eq 4 then $
      p_surf_tm3 = total(p_surf_tm3,4)/n_elements(p_surf_tm3(0,0,0,*))
    ...
    ;-- Calcul pression niveaux interm et Dp
    p_interm_tm3 = fltarr(nlon_tm3,nlat_tm3,nlev+1)
    delta_p_tm3  = fltarr(nlon_tm3,nlat_tm3,nlev)
    for n=0,nlev do begin
        p_interm_tm3(*,*,n) = bp(n) * p_surf_tm3 + ap(n) 
        if n gt 0 then delta_p_tm3(*,*,n-1) = p_interm_tm3(*,*,n-1)-p_interm_tm3(*,*,n)
    endfor

    ;-- Calcul pression mid 
    p_mid_tm3 = fltarr(nlon_tm3,nlat_tm3,nlev)
    for n=0,nlev-1 do p_mid_tm3(*,*,n) = (p_interm_tm3(*,*,n)+p_interm_tm3(*,*,n+1)) / 2.
...
for im=0L,nmon-1 do begin
    ...
    for itr=0,ntrac-1 do begin
        ...
        ;--- Verifie si le traceur est un traceur de tm3
        if trac(itr) eq 'tm3' or trac(itr) eq 'tm3_new' then begin
            p_mid = p_mid_tm3
            p_surf = p_surf_tm3
            p_interm = p_interm_tm3
            delta_p = delta_p_tm3
        endif else begin
            p_mid = p_mid_lmdz
            p_surf = p_surf_lmdz
            p_interm = p_interm_lmdz
            delta_p = delta_p_lmdz
        endelse
	  ...
	  idate_mod = transpose(model.idate)
        ...
        if all_level and all_time then begin
            for no=0L,npt-1 do begin
                ...
                vert_lev(no2,*) = p_mid(pos.lon(no2),pos.lat(no2),*)
            endfor 
            ...
            for no=0L,npt-1 do begin
                ...
                if special_lev then $
                  vert_lev(no2) = peyl_holton(alt(no2),/noinfo,/reverse,$
                                              psurf=p_surf(pos.lon(no2),pos.lat(no2))) $
                else vert_lev(no2) = level(no2)
                vert_lev(no2) = vert_lev(no2) < $
                  p_mid(pos.lon(no2),pos.lat(no2),0)
                vert_lev(no2) = vert_lev(no2) > $
                  p_mid(pos.lon(no2),pos.lat(no2),nlev_lmdz-1)
                
                for t=0L,ntmod-1 do data(no2,t1+t,itr) = $
                  interpol(model.val(pos.lon(no2),pos.lat(no2),*,t), $
                           p_mid(pos.lon(no2),pos.lat(no2),*), $
                           vert_lev(no2))
            ...
            for no=np1,npt-1 do begin
                ...
                if all_level then begin
                    ...
                    if special_lev then $
                      vert_lev(no2) = peyl_holton(alt(no2),/noinfo,/reverse,$
                             psurf=p_surf(pos.lon(no2),pos.lat(no2),0)) $
                    else vert_lev(no2) = level(no2)
                    vert_lev(no2) = vert_lev(no2) < $
                      p_mid(pos.lon(no2),pos.lat(no2),0)
                    vert_lev(no2) = vert_lev(no2) > $
