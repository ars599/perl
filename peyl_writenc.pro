...
;                  gattr_name=gattr_name, $
;                  gattr_value=gattr_value
;
;
...
;
;          gattr_name  : tableau du nom des attributs globaux (string)
;          gattr_value : tableau des valeurs des attributs globaux (string)
;
;          no_dim : set to 1 to suppres output of var dimension
;
;          var_misval: valeur de la "missing value" (float,integer)
;
;          append: set to 1 to append variables to an existing nc file. The dimensions of
;                  the appended variables must have been already defined.
; OUTPUTS: Netcdf file
;
...
if (n_elements(var_title) lt nb_var) then message,'var_title mal dimensionne'
if (n_elements(var_units) lt nb_var) then message,'var_units mal dimensionne'

if append then add_dim=0

;--------------------------------------------------------
; Ouverture Fichier
;--------------------------------------------------------
if append then begin
   ncid_out=ncdf_open(file,/WRITE)
   ncdf_info = ncdf_inquire(ncid_out)
   ncdf_control,ncid_out,/redef
endif else ncid_out=ncdf_create(file,/clobber)


;--------------------------------------------------------
; Quelques attributs globaux
;--------------------------------------------------------
if keyword_set(gattr_name) or keyword_set(gattr_value) then begin

                                ;-- verification des dimensions
    if keyword_set(gattr_name) and keyword_set(gattr_value) $
      and ( n_elements(gattr_name) ne n_elements(gattr_value) ) then begin
        print, ' ** ERROR ** the number of elements for the name and values '+$
          'of the global attributes must be the same'
        print, '             => STOP'
        STOP
    endif
                                ;-- Definition des champs si manque!
    natt = max([n_elements(gattr_name), n_elements(gattr_value)])
    if (not keyword_set(gattr_name)) then begin
        gattr_name=replicate('ATT',natt)
        for n=1,natt-1 do gattr_name(n)=gattr_name(n)+auto_string(n,0)
    endif    
    if (not keyword_set(gattr_value)) then begin
        gattr_value=replicate('---',natt)
    endif

                                ;-- verification du type et
                                ;   conversion.. 
    sname = size(gattr_name)
    svalue = size(gattr_value)    
    if ( (sname)[n_elements(sname)-2] NE 7 ) then begin
        print, ' ** WARNING ** the TYPE of the global attribute names must STRING'
        print, ' ** WARNING ** TYPE is then converted into STRING'
        gattr_name = STRING(gattr_name)
    endif    
    if ( (svalue)[n_elements(svalue)-2] NE 7 ) then begin
        print, ' ** WARNING ** the TYPE of the global attribute values must STRING'
        print, ' ** WARNING ** TYPE is then converted into STRING'
        gattr_value = STRING(gattr_value)
    endif
    
                                ;-- ecriture
    for i=0,natt-1 do begin
        ncdf_attput, ncid_out, /GLOBAL, gattr_name[i], gattr_value[i]
    endfor    
...
if n_elements(add_dim) eq 0 then add_dim_loc = replicate(1,nb_dim) else begin
    if n_elements(add_dim) ge nb_dim then add_dim_loc = add_dim(0:nb_dim-1) $
      else add_dim_loc = replicate(add_dim(0),nb_dim)
endelse

;----- Cas specific fichier append: on relit aussi les dimensions
;      existantes...
if append then begin
    nb_dim_old = ncdf_info.ndims
    dim_name_old = strarr(nb_dim_old)
    dim_size_old = intarr(nb_dim_old)
    for i=0,nb_dim_old-1 do begin
        ncdf_diminq,ncid_out,i,name,dd
        dim_name_old(i)=name & dim_size_old(i)=dd
    endfor
endif 
...
if special_string then begin
    dim_size  = [dim_size,strlen_max]
    dim_name  = [dim_name,'len']
    dim_title = [dim_title,'string lenght']
    dim_units = [dim_units,'-']
    tab_dim_string = findgen(strlen_max)+1.
    nb_dim = nb_dim + 1
    add_dim_loc = [add_dim_loc,0]
endif
;print,dim_size
...
if append then begin
    for d=0,nb_dim-1 do begin
        foo = where(dim_size_old eq dim_size(d),cc)
        if cc ne 1 then message,'adding variable not possible: dimension not found!'        
        dim_id(d) = foo(0)
    endfor 
endif else begin
   for d=0,nb_dim-1 do begin
...
if total(add_dim_loc) gt 0 then begin

    nb_max  = max(dim_size)

    if (not keyword_set(dim_val)) then begin
        cmde = 'dim_val = {D0:dblarr(dim_size(0))+1.'
        for d=1,nb_dim-special_string-1 do cmde = cmde + $
        ...
        if (nn le 0) then begin
            for d=0,nb_dim-special_string-1 do begin
                nd = dim_size(d)
                if d eq 0 then cmde = 'dim_val = {D0:dim_val(0:'+auto_string(nd-1,0)+',0)' else $
                  cmde = cmde + ',D'+auto_string(d,0) + $
                  ':dim_val(0:'+auto_string(nd-1,0)+','+auto_string(d,0)+')'
            endfor
            cmde = cmde + '}'
            res = execute(cmde)

                                ;--- Dim_val sous forme de structure
                                ;    deja fournie (check dimension)
        endif else begin
            if nn ne (nb_dim-special_string) then $
              message,'! Problem dim_val structure mal definiee...'
            for d=0,nb_dim-special_string-1 do begin
                res = execute('temp = n_elements(dim_val.' + tag(d)+')')
                if temp ne dim_size(d) then $
                  message,'! Problem de dimension pour tag '+tag(d)
            endfor
        endelse
    endelse
...
for d=0,nb_dim-1 do begin   
    ...
    if add_dim_loc(d) then begin
        tag = tag_names(dim_val)
        
        if special_string and d eq (nb_dim-1) then begin
            ss = size(tab_dim_string)
        endif else begin
            res = execute('ss = size(dim_val.' + tag(d)+')')
        endelse
        ...
        if ccc eq 1 then begin
            print,'PROBLEME dimension de type string : Sauvers les valeurs dans une variable speciale et non dans une dimension...'
        endif
...
;    print,var_name(n),dim_id(*),bbb,sss,lll,fff,ddd,ccc
    ...
    if string_flag and special_string eq 0 then message,'Hummmm...'
    ...
    if ((ndd+string_flag) eq 0) then goto,suite_defvar
                                ...
                                ;    variable. !! cas special pour les
                                ;    string !! Last_dim est tjs la dim de
                                ;    strlenght! 
    if NOT keyword_set(var_dim) then begin
        ...
        if string_flag then begin
            ...
            res  = execute(cmde)
            dd   = max(len)
            if dd gt dim_size(nb_dim-1) then message,'HHUUMM, dim bizarre pour var string!!'
            jj_var_dim(0) = nb_dim-1
        endif
        for d=0,ndd-1 do begin            
            ...
            if cc gt 1 then begin
                if ii(cc-1) eq (nb_dim-1) and cc le 2 then ii = ii(0) else $
        ...
        res  = execute(cmde)
        if string_flag then jj_var_dim = [nb_dim-1,jj_var_dim]
        ...
        for l=0,ll-1 do begin
            if jj_var_dim(l) ge nb_dim then $
              message,'Probleme dans var_dim.'+tags(n)+' pour '+var_name(n)
            if (string_flag eq 0 and dim_size(jj_var_dim(l)) ne ss(l+1)) or $
              (string_flag eq 1 and l gt 0 and  dim_size(jj_var_dim(l)) ne ss(l)) then $
              message,'Probleme de dim pour variable '+var_name(n)            
        endfor 
        ...
        if string_flag then begin
            ...
            if dim_size(jj_var_dim(0)) lt dd then $ 
              message,'ERR: Var STRING, Mais nb char SUPERIEUR a dimension strlen !!'
        endif
    ...
    if (keyword_set(var_misval)) then begin
        if n_elements(var_misval) ge nb_var then missval = var_misval(n) else $
          missval = var_misval(0)
        if (abs(missval) gt 1.e34) then begin
            missval=1.e34
            if n eq 0 then print,'Warning Missing Value to large and reset to : ',missval
        endif
        ncdf_attput,ncid_out,var_id(n),'missing_value',missval
    endif
endfor
...
for d=0,nb_dim-1 do begin
    if add_dim_loc(d) then begin
        tag = tag_names(dim_val)
        if special_string and d eq (nb_dim-1) then begin
            ncdf_varput,ncid_out,dim_val_id(d),tab_dim_string(*) 
        endif else begin
            cmde = 'ncdf_varput,ncid_out,dim_val_id(d),dim_val.'+tag(d)
            res = execute(cmde)
        endelse
    endif
endfor
