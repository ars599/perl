...
;
; CALLING SEQUENCE:
;	CCG_FSREAD,file='filename',arr
;	CCG_FSREAD,file='/home/gcarb/data/co/flask/site/bme.co',data
;	CCG_FSREAD,file='/users/ken/mlo.co2',structarr
;
; INPUTS:
...
;
; OUTPUTS:
;	Data:		This array is of type 'ccgfixed' structure:
;
;			arr().str 	-> entire site string
;			arr().x		-> UTC date/time in decimal format
;			arr().yr	-> UTC sample collection year
;			arr().mo	-> UTC sample collection month
;			arr().dy	-> UTC sample collection day
;			arr().hr	-> UTC sample collection hour 
;			arr().mn	-> UTC sample collection minute
...
;			arr().ayr	-> sample analysis year (local time)
;			arr().amo	-> sample analysis month (local time)
;			arr().ady	-> sample analysis day (local time)
;			arr().ahr	-> sample analysis hour  (local time)
;			arr().amn	-> sample analysis minute (local time)
...
;	of the site file.  
;		Example:
;			CCG_FSREAD,file='/home/gcarb/data/co2/flask/site/brw.co2',arr
;			.
;			.
;			.
;			PLOT, arr.x,arr.y
...
;-
;
PRO CCG_FSREAD,	file=file,$
		all=all, noheader=noheader,$
...
IF NOT KEYWORD_SET(file) THEN BEGIN
	...
	CCG_MESSAGE,"(ex) ccg_fsread,file='/home/gcarb/data/ch4/flask/site/brw.ch4',arr"
	RETURN
ENDIF
;
...
(STRPOS(file,"co2o18") NE -1): 	BEGIN
					DEFAULT=(-999.999)
					header=1
					dp='3'
				END
(STRPOS(file,"co2c13") NE -1): 	BEGIN
					DEFAULT=(-999.999)
					header=1
					dp='3'
				END
(STRPOS(file,"ch4c13") NE -1): 	BEGIN
					DEFAULT=(-999.999)
					header=1
					dp='3'
				END
...
ELSE:				BEGIN
					...
					CCG_MESSAGE,$
					"Codes include co2,ch4,co,h2,co2c13,co2o18,ch4c13."
					RETURN
				END
ENDCASE
;
IF KEYWORD_SET(noheader) THEN header=0
;
	...
	ENDIF ELSE BEGIN
		...
		use=1
			...
			arr[i].mn=mn
		 	...
		 	arr(i).inst=in
		 	arr(i).flag=fg
			CCG_DATE2DEC,yr=ayr,mo=amo,dy=ady,hr=ahr,mn=amn,dec=dec
 		 	arr(i).adate=dec 
			arr[i].ayr=ayr
			arr[i].amo=amo
			arr[i].ady=ady
