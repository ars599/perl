...
sp=STRLOWCASE(sp)
IF sp(0) EQ 'all' THEN $
	sp=['co2','ch4','co','h2','n2o','sf6','co2c13','co2o18','ch4c13']
nsp=N_ELEMENTS(sp)
...
file=netdir+site
CCG_NSREAD,	file=file,ns
data=CREATE_STRUCT('net',ns)

FOR isp=0,nsp-1 DO BEGIN
	;
	;Read appropriate site files
	;
	;Must deal with exceptions to the rule
	;
	CASE sp[isp] OF
	'co2c13': file=sitedir+'silco2/flask/site/'+site+'.c13'
	'co2o18': file=sitedir+'silco2/flask/site/'+site+'.o18'
	ELSE:     file=sitedir+sp(isp)+'/flask/site/'+site+'.'+sp(isp)
	ENDCASE

	CCG_FSREAD,file=file,nomessages=nomessages,zz
...
;
IF NOT KEYWORD_SET(dev) THEN WINDOW,2,XSIZE=700,YSIZE=900,TITLE='CCG_FLASKVU'
;
;try out cursor
;
xx1=xdatamin
...
ENDIF ELSE BEGIN
	WHILE forever DO BEGIN
		...
		IF !MOUSE.BUTTON EQ 1 THEN BEGIN
			...
			cursor_position=(xd0-xx1)/(xx2-xx1)
		
			IF cursor_position GE 0 AND cursor_position LE 1 THEN BEGIN
				q=MIN(ABS(data.(0).x-xd0),z)
				j=WHERE(data.(0).x EQ data.(0)[z].x)
			;
			;Send network site details 
			;
			CCG_MESSAGE,' '
			FOR z=0,N_ELEMENTS(j)-1 DO CCG_MESSAGE,$
				STRING(FORMAT='(A6,1X,A)','event',data.(0)[j(z)].str)
			highlight=data.(0)[j(0)].x
			ENDIF ELSE forever=0
		ENDIF
