...
;       Modified, MC, January 2001, changed working path to /tmp, srcfile append _$USER
;-
PRO	 MC_CCGVU,	x=x,y=y,$
			npoly=npoly,nharm=nharm,tzero=tzero,$
			interval=interval,cutoff1=cutoff1,cutoff2=cutoff2,$
			even=even,$
			ftn=ftn,sc=sc,tr=tr,gr=gr,$
...
destdir='/tmp/'
user=GETENV("USER")
srcfile=destdir+'mc_ccgvu.data_'+user
;ccode='/crc/b/gaslab/contrib/crvfit/crvfit'
;ccode='/home/ccg/ken/contrib/crvfit/crvfit'
newpath='/home/data02/cuntz/data/noaa/bin/crvfit/'
...
ccode=newpath+'crvfitLinux.obelix' $
else $
ccode=newpath+'crvfitLinux.asterix'
endif else if (os eq 'osf') then $
  ccode=newpath+'crvfitDEC' $
else if (os eq 'aix') then $
  ccode=newpath+'crvfitIBM' $
else begin
    print,'Error MC_CCGVU: operating sytem not supported yet.'
    !QUIET=q
    return
endelse
;
...
;
SPAWN,	'limit stacksize unlimited; '+ccode+arg, /sh
;
...
;
SPAWN,	'\rm -f '+srcfile+' '+destdir+'zzz_*.zzz', /sh
;
