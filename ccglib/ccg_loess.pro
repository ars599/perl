...
;LOESSPACK routines.
;
;ccode='/crc/b/gaslab/contrib/loess/ccg_loess'
;ccode='/users/ken/contrib/loess/ccg_loess'
host = getenv('HOST')
if (host eq 'odalix' or host eq 'alambix' or host eq 'appatix') then $
   ccode='/home/gcarb/contrib/idllib/ccglib/contrib/loess/ccg_loess' $
else begin
   print,'!!!! Loess ne mache pas sur DEC....
   print,'Hit .continue to continue anyway...'
   stop
endelse
...
;
SPAWN,	'rm -f '+' '+reportfile+' '+fitfile+' '+datfile+' '+tempfile
END
