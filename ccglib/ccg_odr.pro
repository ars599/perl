...
;			.
;			CCG_FREAD,$
;			file='/home/gcarb/data/co/maps/csiro/csiro.cal',nc=4,skip=1,data
;			;
;			x=data(0,*)
;			xwt=1.0/data(1,*)
;			y=data(2,*)
...
IF NOT KEYWORD_SET(xwts) AND NOT KEYWORD_SET(ywts) THEN BEGIN 
	CCG_MESSAGE,'No weights assigned, assuming a least squares fit ...'
...
;
;ccode='/users/ken/contrib/odr/odrfit'
ccode='/home/gcarb/contrib/idllib/ccglib/contrib/odr/odrfit'
...
;
CCG_SREAD,file=reportfile,sarr
;
...
;
CCG_FREAD,file=tempfile,nc=3,var
;
np=N_ELEMENTS(var(0,*))
...
FOR i=0,np-1 DO BEGIN
	PRINT,FORMAT='(I10,1X,F10.6,1X,F10.6)',i,results(0,i),results(1,i)
ENDFOR
;
...
;
SPAWN,	'rm -f '+tempfile+' '+reportfile
END
