...
;
; CALLING SEQUENCE:
;	CCG_FLASKAVE_NEW,sp='o18',site='asc',xret,yret,xnb,ynb,xrej,yrej
;	CCG_FLASKAVE_NEW,sp='co2',site='brw',xret,yret,xnb,ynb,xrej,yrej
;	CCG_FLASKAVE_NEW,sp='co2',file='test',nomessages=1,xret,yret,xnb,ynb,xrej,yrej
;
; INPUTS:
...
;
; PROCEDURE:
;	CCG_FLASKAVE_NEW may be called from the IDL command line or
...
;		or
;
;			CCG_FLASKAVE_NEW,sp='co',file='/users/ken/test',xret,yret
;			PLOT, 	xret,yret,PSYM=6,COLOR=pen(2)
;		
...
sformat=$
	'(A3,1X,I4.4,1X,4(I2.2,1X),A8,1X,A1,1X,A9,1X,F8.3,'+$
...
WHILE NOT EOF(unit) DO BEGIN
	...
	ENDIF ELSE BEGIN
		READF, unit, FORMAT=sformat,  $
			st,yr,mo,dy,hr,mn,fi,me,spec,mr,fg,in,ayr,amo,ady,ahr,amn
...
PRO CCG_FLASKAVE_NEW,	site=site,$
			sp=sp,$
			file=file,$
			header=header,$
			skip=skip, $
...
IF NOT KEYWORD_SET(file) AND NOT KEYWORD_SET(site) THEN BEGIN
	...
	CCG_MESSAGE,"ex:  CCG_FLASKAVE_NEW, site='smo',sp='ch4'"
	RETURN
ENDIF
;
...
;
CCG_FSREAD2,file=file,sp=sp,header=header,skip=skip,nomessages=nomessages,nlines=nlines,x,y,flag
;
