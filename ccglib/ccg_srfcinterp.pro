...
;	Example:
;		IDL> CCG_SRFCINTERP,$
;		IDL> srfcfile='/home/gcarb/data/dei/ext/co2/results.ccg.1998/ref_mbl.mtx',$
...
WHILE NOT EOF(unit) DO BEGIN
	READF,unit,FORMAT='(F12.6,41(1X,F12.4))',a,b
	srfc_x(i)=a
        srfc_z(i,*)=b
...
IF KEYWORD_SET(site) THEN BEGIN
	...
	FOR i=0,N_ELEMENTS(site)-1 DO BEGIN
		...
		CCG_SITEDESC,site=site[i],sitedesc
...
OPENW,fpdebug,'/tmp/ccg_srfcinterp.debug',/GET_LUN
;
...
FOR i=0,ninterp-1 DO BEGIN
	...
	IF px1(0) NE -1 AND px2(0) NE -1 AND py1(0) NE -1 AND py2(0) NE -1 THEN BEGIN
		...
		PRINTF,fpdebug,FORMAT='(3(F12.6))',x1,interp_x(i),x2
		PRINTF,fpdebug,FORMAT='(3(F12.3))',y1,interp_y(i),y2
		PRINTF,fpdebug,FORMAT='(4(F12.3))',srfc_z(px1,py1),$
		...
		PRINTF,fpdebug,FORMAT='(2(F12.3))',z_at_xnot_ynot,z_at_ynot_xnot
		;
